...
import { 
  ...
  Layers, ShieldCheck, Zap, Copy, Check, Maximize2, Minimize2, RefreshCw, 
  Globe, Server, Database, Hash, MousePointer2, Bell, Clock, TrendingUp, BarChart3, 
  ShieldAlert, Search, Box, Filter 
} from "lucide-react";
  ...
  return (
    <div className={cn("rounded-2xl border bg-white shadow-sm overflow-hidden flex flex-col", className)}>
      <div className="p-6 flex-1">
        <div className="flex items-center justify-between pb-3"><h3 className="text-[10px] font-black uppercase tracking-[0.1em] text-zinc-400">{title}</h3>{Icon && <Icon className="h-4 w-4 text-zinc-300" />}</div>
...
function CopyableText({ text, label }: { text: string, label?: string }) {
  ...
  return (
    <div className="group relative flex flex-col gap-1 w-full bg-zinc-50/50 p-2.5 rounded-xl border border-zinc-100 hover:border-zinc-200 transition-all">
      ...
      <div className="flex items-start justify-between gap-4"><div className="text-[11px] font-mono font-bold text-zinc-800 break-all leading-relaxed flex-1">{text}</div><button onClick={onCopy} className="p-1 hover:bg-white rounded-lg border border-transparent hover:border-zinc-200 transition-all text-zinc-400 hover:text-zinc-900 shrink-0 mt-0.5 shadow-sm">{copied ? <Check size={12} className="text-green-600" /> : <Copy size={12} />}</button></div>
    </div>
  );
}

// --- Detail Sections ---
function VncConsole({ namespace, name }: { namespace: string, name: string }) {
  ...
  useEffect(() => {
    ...
    const startVnc = () => {
      try {
        ...
        if (typeof RFBConstructor !== "function") { setErrorMsg("VNC 引擎载入失败：找不到有效的构造函数。"); setStatus("error"); return; }
        rfb = new RFBConstructor(containerRef.current!, wsUrl, { wsProtocols: ["binary"] });
  ...
  return (
    <div className={cn("bg-zinc-900 shadow-2xl overflow-hidden flex flex-col transition-all duration-300 relative", isTheaterMode ? "fixed inset-0 z-[100] h-screen w-screen rounded-none" : "rounded-2xl border border-zinc-800 h-[calc(100vh-280px)] min-h-[600px] w-full")}>
      <div className="flex items-center justify-between gap-2 bg-zinc-900 border-b border-zinc-800 p-3 px-5">
         ...
         {status === "connected" && (
           <div className="flex gap-2">
              <button onClick={() => setShowCADModal(true)} title="发送 Ctrl+Alt+Del" className="flex items-center gap-2 px-3 py-1 bg-red-950/20 hover:bg-red-500 text-red-400 hover:text-white rounded-md text-[10px] font-bold uppercase transition-all border border-red-500/30 shadow-sm active:scale-95"><ShieldAlert size={12} /> Ctrl+Alt+Del</button>
      ...
      {showCADModal && (
        <div className="absolute inset-0 z-[200] flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-md">
           <div className="bg-white rounded-3xl shadow-2xl border border-zinc-100 max-w-sm w-full p-8">
              <div className="flex flex-col items-center text-center space-y-4">
                 ...
                 <p className="text-zinc-500 text-xs font-medium leading-relaxed">发送 Ctrl+Alt+Del 可能会导致虚拟机系统立即重启。请确认已保存所有工作。</p>
                 <div className="flex flex-col w-full gap-3 pt-4">
                    <button onClick={sendCAD} className="w-full py-3 bg-red-600 hover:bg-red-700 text-white rounded-2xl text-[11px] font-black uppercase transition-all shadow-lg shadow-red-200 active:scale-95">确认发送</button>
      ...
      {status === "error" && (
        <div className="absolute inset-0 top-12 bg-zinc-950/90 backdrop-blur-sm flex items-center justify-center p-8 z-10 text-center">
           <div className="max-w-md space-y-4"><div className="bg-red-500/10 p-4 rounded-full w-16 h-16 mx-auto flex items-center justify-center"><ShieldAlert className="text-red-500" size={32} /></div><h3 className="text-white font-black uppercase tracking-widest text-sm">VNC 连接失败</h3><p className="text-zinc-400 text-xs font-medium leading-relaxed">{errorMsg || "无法建立 VNC 握手。"}</p><button onClick={() => window.location.reload()} className="px-6 py-2 bg-white text-zinc-950 rounded-xl text-[10px] font-black uppercase hover:bg-zinc-200 transition-all">重试</button></div>
        </div>
      )}
    </div>
  );
...
function SerialConsole({ namespace, name }: { namespace: string, name: string }) {
  ...
  const syncSize = () => { if (wsRef.current?.readyState === WebSocket.OPEN && xtermRef.current && fitAddonRef.current) { fitAddonRef.current.fit(); const term = xtermRef.current; wsRef.current.send(`\x15export TERM=xterm-256color LANG=C.UTF-8; stty cols ${term.cols} rows ${term.rows}\r`); term.focus(); } };
  const toggleTheaterMode = () => setIsTheaterMode(!isTheaterMode);
  useEffect(() => {
    ...
    const fitAddon = new FitAddon(); term.loadAddon(fitAddon); term.open(termRef.current); fitAddon.fit();
    xtermRef.current = term; fitAddonRef.current = fitAddon;
    const protocol = window.location.protocol === "https:" ? "wss:" : "ws:";
    const ws = new WebSocket(`${protocol}//${window.location.host}/api/v1/ws?namespace=${namespace}&vmi=${name}&type=serial&context=${getContext()}`);
    ws.binaryType = 'arraybuffer'; wsRef.current = ws;
    ws.onopen = () => { setConnStatus("connected"); term.clear(); term.focus(); };
    ws.onmessage = (e) => { if (e.data instanceof ArrayBuffer) term.write(new TextDecoder().decode(e.data)); else if (typeof e.data === "string" && e.data.startsWith("console error")) setConnStatus("error"); };
    ws.onclose = () => setConnStatus("closed"); ws.onerror = () => setConnStatus("error");
    term.onData((data) => { if (ws.readyState === WebSocket.OPEN) ws.send(data); });
    const handleResize = () => fitAddon.fit(); window.addEventListener("resize", handleResize);
    return () => { ws.close(); term.dispose(); window.removeEventListener("resize", handleResize); };
  }, [namespace, name]);
  return (
    <div className={cn("bg-zinc-950 shadow-2xl overflow-hidden flex flex-col transition-all duration-300", isTheaterMode ? "fixed inset-0 z-[100] h-screen w-screen rounded-none" : "rounded-2xl border border-zinc-800 h-[calc(100vh-280px)] min-h-[600px] w-full")}>
      <div className="flex items-center justify-between gap-2 bg-zinc-900 border-b border-zinc-800 p-3 px-5">
         <div className="flex items-center gap-3">
            <div className={cn("w-2.5 h-2.5 rounded-full", connStatus === "connected" ? "bg-green-500 animate-pulse" : connStatus === "error" ? "bg-red-500" : "bg-zinc-600")} />
         ...
         <div className="flex gap-2">
            ...
            <button onClick={toggleTheaterMode} className="flex items-center gap-2 px-3 py-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-md text-[10px] font-bold uppercase transition-all border border-zinc-700">{isTheaterMode ? <Minimize2 size={12} /> : <Maximize2 size={12} />} {isTheaterMode ? "Exit" : "Theater"}</button>
         </div>
      </div>
...
function VMList() {
  ...
  const fetchVms = async () => {
    setLoading(true);
    try {
      const res = await apiFetch(`/api/v1/vms?name=${searchTerm}&status=${statusFilter}&namespace=${nsFilter}`);
      const data = await res.json(); setVms(data.items || []);
...
function VMDetailContent() {
  ...
  const fetchData = async () => {
    try {
      ...
      let strat = mStrategy; 
      if (strat === null) {
        // Silent Discovery: check /apis which always returns 200
        const apisRes = await apiFetch("/apis");
        if (apisRes.ok) {
          const apis = await apisRes.json();
          const hasVmiMetrics = apis.groups?.some((g: any) => g.name === "metrics.kubevirt.io");
          strat = hasVmiMetrics ? "vmi" : "pod";
          setMStrategy(strat);
        }
      }

      if (strat === "vmi") {
        const m = await apiFetch(`/apis/metrics.kubevirt.io/v1beta1/namespaces/${namespace}/virtualmachineinstances/${name}`);
        if (m.ok) {
          const d = await m.json(); 
          setMetrics(p => [...p.slice(-19), { timestamp: Date.now(), time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }), cpuUsage: d.status?.cpu?.usageCores || 0, memoryUsage: (d.status?.memory?.usageBytes || 0) / (1024 * 1024) }]);
        }
      } else if (strat === "pod") {
        const ps = await apiFetch(`/api/v1/namespaces/${namespace}/pods?labelSelector=kubevirt.io/domain=${name}`);
        if (ps.ok) {
          const pod = (await ps.json()).items?.[0];
          if (pod) {
            const pm = await apiFetch(`/apis/metrics.k8s.io/v1beta1/namespaces/${namespace}/pods/${pod.metadata.name}`);
            if (pm.ok) {
              const d = await pm.json();
              const cpu = d.containers?.[0]?.usage?.cpu || "0n";
              const mem = d.containers?.[0]?.usage?.memory || "0Ki";
              const pCpu = (c:string) => c.endsWith("n") ? parseInt(c)/1e9 : c.endsWith("u") ? parseInt(c)/1e6 : c.endsWith("m") ? parseInt(c)/1e3 : parseInt(c);
              const pMem = (m:string) => m.endsWith("Ki") ? parseInt(m)/1024 : m.endsWith("Mi") ? parseInt(m) : m.endsWith("Gi") ? parseInt(m)*1024 : parseInt(m)/(1024*1024);
              setMetrics(p => [...p.slice(-19), { timestamp: Date.now(), time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }), cpuUsage: pCpu(cpu), memoryUsage: pMem(mem) }]);
            }
          }
        }
      }
  ...
  return (
    <div className={cn("space-y-6 animate-in fade-in duration-500", activeTab === "console" || activeTab === "vnc" ? "max-w-full" : "")}>
      <div className="flex items-center justify-between border-b pb-6 border-zinc-100"><div className="flex items-center gap-4"><button onClick={() => navigate("/vms")} className="p-2 hover:bg-zinc-100 rounded-lg border border-zinc-200 transition-all bg-white shadow-sm"><ChevronLeft size={18} /></button><div><h2 className="text-2xl font-bold tracking-tight text-zinc-900">{vm.metadata.name}</h2><div className="flex items-center gap-3 mt-1 text-xs"><span className="font-bold text-zinc-600">{vm.metadata.namespace}</span><StatusBadge status={vm.status?.printableStatus}/></div></div></div><div className="flex gap-2"><button onClick={() => handleAction('start')} className="px-4 py-2 bg-zinc-950 text-white rounded-lg text-xs font-bold hover:bg-zinc-800 shadow-md transition-all">Start</button><button onClick={() => handleAction('stop')} className="px-4 py-2 bg-white border border-red-100 rounded-lg text-xs font-bold hover:bg-red-50 text-red-600 shadow-sm transition-all">Stop</button><button onClick={() => handleAction('restart')} className="px-4 py-2 bg-white border border-zinc-200 rounded-lg text-xs font-bold hover:bg-zinc-50 shadow-sm transition-all">Restart</button></div></div>
...
function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-zinc-50/20">
      <aside className="w-72 border-r bg-white flex flex-col fixed inset-y-0 z-50 shadow-sm"><div className="h-16 border-b flex items-center px-8 gap-4"><div className="bg-zinc-900 p-1.5 rounded-lg"><Terminal className="text-white h-4 w-4" /></div><span className="font-black text-lg tracking-tighter uppercase text-zinc-900 leading-none">Virt<br/><span className="text-zinc-400">Dashboard</span></span></div><div className="p-6 pb-2"><div className="bg-zinc-50 border border-zinc-200 rounded-xl p-3"><label className="text-[9px] font-black text-zinc-400 uppercase tracking-widest block mb-2 px-1">Cluster</label><div className="relative"><select value={currentCtx} onChange={hCC} className="w-full bg-white border border-zinc-200 rounded-lg px-3 py-2 text-[11px] font-bold outline-none appearance-none cursor-pointer pr-8">{contexts.map(c => <option key={c} value={c}>{c}</option>)}</select><div className="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none text-zinc-400"><Globe size={12} /></div></div></div></div><nav className="flex-1 p-6 pt-2 space-y-1.5 overflow-y-auto">{[ { name: "Overview", path: "/", icon: LayoutDashboard }, { name: "Machines", path: "/vms", icon: Cpu }, { name: "Storage", path: "/dvs", icon: HardDrive } ].map((item) => ( <Link key={item.path} to={item.path} className={cn("flex items-center gap-4 px-5 py-3.5 rounded-2xl transition-all text-[11px] font-bold uppercase tracking-wider", (item.path==="/"?location.pathname==="/":location.pathname.startsWith(item.path)) ? "bg-zinc-900 text-white shadow-xl" : "text-zinc-400 hover:bg-zinc-50 hover:text-zinc-900")}><item.icon size={18} /> {item.name}</Link> ))}</nav><div className="p-8 border-t"><div className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.3em]">Stable v1.6.0</div></div></aside>
